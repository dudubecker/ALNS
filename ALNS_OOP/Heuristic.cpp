...
Sol Heuristic::aplicar(Sol &S){
	
	// O método "aplicar" irá executar o método específico de cada heurística
	
	aplicarMetodoEspecifico(S);
	
	return S;
	
}

//// Implementação - RemovalHeuristic

int RemovalHeuristic::inicializarMetodo(Sol &S) {
	...
	// Bug -> quando o número de pedidos a serem removidos é maior do que o número de pedidos existentes
	mi = std::min(mi, S.A_size);
...
Sol RandomRemoval::aplicarMetodoEspecifico(Sol &S){
	
	int mi = RemovalHeuristic::inicializarMetodo(S);
...
Sol WorstRemoval::aplicarMetodoEspecifico(Sol &S) {
	...
	while (n_pedidos_removidos < mi){
		...
		for (auto pedido: pedidos){
			...
			if (eta > 0){
				...
				double noise = (-1.0 + (2.0 * rand_value / RAND_MAX))*(eta)*(S.inst.max_dist);
...
Sol ShawsRemoval::aplicarMetodoEspecifico(Sol &S) {
...
//// Implementação - InsertionHeuristic

void InsertionHeuristic::inicializarMetodo() {
	
	;
	
}

Sol GreedyInsertion::aplicarMetodoEspecifico(Sol &S) {
	
	// Inicialização da inserção:
	
	InsertionHeuristic::inicializarMetodo();
	
	// Variável que controlará o número de pedidos inseridos pelo algoritmo
	int qtd_inseridos {0};
	
	// Quantidade de pedidos não atendidos na solução 
	int qtd_pedidos_nao_atendidos {S.L_size};
	...
	while (true){
		...
		for (auto &pedido: S.L){
			...
			std::vector<double> dados_melhor_insercao = S.calcularDadosMelhorInsercao(pedido);
		...
		if (delta_min < 9999){
			...
			S.inserirPedido(pedido_min, dados_melhor_insercao_min.at(1), dados_melhor_insercao_min.at(2), dados_melhor_insercao_min.at(3));
...
Sol RegretInsertion::aplicarMetodoEspecifico(Sol &S) {
	...
	while (true){
		...
		for (auto &pedido: pedidos_nao_atendidos){
			...
			for (auto index_rota {0}; index_rota < m; index_rota++){
				...
				std::vector<double> dados_melhor_insercao_rota = S.calcularDadosMelhorInsercao(pedido, index_rota);
		...
		if (qtd_insercoes_factiveis_total > 0){
			...
			if (pedido_min_insercoes == 0){
				...
				S.inserirPedido(pedido_max_regret, dados_melhor_insercao_min.at(1), dados_melhor_insercao_min.at(2), dados_melhor_insercao_min.at(3));
			...
			// Caso contrário, isso significará que deve-se optar pelo pedido com o mínimo de posições de inserção encontradas!
				...
				S.inserirPedido(pedido_min_insercoes, dados_melhor_insercao_min.at(1), dados_melhor_insercao_min.at(2), dados_melhor_insercao_min.at(3));
				qtd_inseridos += 1;
	...
	for (auto &pedido: pedidos_nao_atendidos){
		...
		S.executarMelhorInsercao(pedido);
