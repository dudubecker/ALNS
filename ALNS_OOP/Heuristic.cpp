		...
		case 'G':{
			...
			while (true){
				...
				for (auto &pedido: S.L){
					...
					double delta = S.delta_melhor_insercao(pedido);
				...
				if (delta_min < 9999){
					...
					S = S.melhor_insercao(pedido_min);
					qtd_inseridos += 1;
		...
		case 'A':{
			...
			while (true){
				...
				for (auto &pedido: S.L){
					...
					for (unsigned index_rota {0}; index_rota < m; index_rota++){
						
						double delta_min = S.delta_melhor_insercao(pedido, index_rota);
					...
					if (qtd_insercoes_factiveis_pedido < (m - k + 1)){
						...
						if (qtd_insercoes_factiveis_pedido == qtd_insercoes_factiveis_pedido_min){
							...
							double delta_min = S.delta_melhor_insercao(pedido);
